...
///  Обработчик распознавания. Процедура срабатывает, когда распознана одна из заданных фраз
procedure OnRecogn(phrase : string);
begin
  {Здесь силами студентов мехмата ЮФУ, уж не помню каких, но точно талантливых, было установлено, 
   что при обработке распознанной фразы не нужно останавливать процесс распознавания функцией recogn.Stop,
   т.к. даже при асинхронном распознавании на время выполнения обработчика процесс распознавания останавливается
   автоматически. Более того, принудительная остановка распознавания на время работы обработчика иногда приводит к 
   ошибкам синхронизации и выбросу исключения.}
  if phrase = capital then
    ...
    begin
      //  Если вариант выбран неверно - отмечаем неверный вариант красным, меняем счёт
      Say('Нет');
      wrong += 1;
      txt[answersCount + 1].Text := 'Счёт : ' + correct.ToString + ' верно из ' + (correct+wrong).ToString;
      for var i:=1 to answersCount do
